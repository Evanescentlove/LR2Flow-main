...
'''
# ----------------------------------------
# affine coupling layer for arbitrary split channels
# ----------------------------------------
'''

class AffineCoupling_(nn.Module):
    def __init__(self, in_channels, split_channels, cond_channels=None, LRvsothers=True, opt=None):
        super().__init__()
        ...
        if LRvsothers:
            f_in_channels = self.split_channels if cond_channels is None else self.split_channels + cond_channels
            f_out_channels = (self.in_channels - self.split_channels) * 2
        else:
            f_in_channels = self.in_channels - self.split_channels if cond_channels is None else self.in_channels - self.split_channels + cond_channels
            f_out_channels = self.split_channels
    ...
    def normal_flow(self, z, u=None, y=None, logdet=None):
        ...
        if self.LRvsothers:
            z1, z2 = torch.split(z, [self.split_channels, self.in_channels-self.split_channels], dim=1)
            h = self.f(z1) if self.cond_channels is None else self.f(thops.cat_feature(z1, u))
            shift, scale = thops.split_feature(h, "cross")
            logscale = 0.318 * torch.atan(2 * scale)
            z2 = (z2 + shift) * torch.exp(logscale)
            ...
            z2, z1 = torch.split(z, [self.split_channels, self.in_channels-self.split_channels], dim=1)
            shift = self.f(z1) if self.cond_channels is None else self.f(thops.cat_feature(z1, u))
            z2 = z2 + shift
